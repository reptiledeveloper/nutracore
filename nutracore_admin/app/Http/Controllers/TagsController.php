...
class TagsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $attributes = Tags::where('is_delete', 0)->orderBy('id', 'desc');
        if (!empty($search)) {
    ...
    public function add(Request $request)
    {
        ...
        if (is_numeric($id) && $id > 0) {
            ...
            $attributes = Tags::find($id);
            if (empty($attributes)) {
                return redirect($this->ADMIN_ROUTE_NAME . '/tags');
            }
        }
        ...
        if ($request->method() == 'POST' || $request->method() == 'post') {
            ...
            if (empty($back_url)) {
                $back_url = $this->ADMIN_ROUTE_NAME . '/tags';
            }
            ...
            if ($createdCat) {
                $alert_msg = 'Tags has been added successfully.';
                if (is_numeric($id) && $id > 0) {
                    $alert_msg = 'Tags has been updated successfully.';
                }
        ...
        $page_heading = 'Add Tags';

        if (is_numeric($id) && $id > 0) {
            $page_heading = 'Update Tags ';
        }
    ...
    public function delete(Request $request)
    {
        ...
        if (is_numeric($id) && $id > 0) {
            $is_delete = Tags::where('id', $id)->update(['is_delete' => 1]);
        }
