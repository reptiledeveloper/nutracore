    ...
    <table style="width: 100%">
        ...
        @foreach ($order_items as $key => $value)
            @php
               ...
               // $image='https://imagesbuycart.reptileantitheft.com/products/uploads/media/2021/Dairy_Milk_Fruit_Nut_Chocolate_Bar_36_Gms_buybuycart.jpg';
                $options = [
    "ssl" => [
        "verify_peer" => false,
...
$imageContent = file_get_contents($image, false, $context);
            @endphp
    ...
    <div style="float: right; margin-right:50px;font-size: 12px">
        ...
        <span>Delivery Charges:  </span> {{$orders->delivery_charges??''}}<br>
        <span>Tax(18%) :  </span> <br>
        <span>Online Amount :  </span> {{$orders->online_amount??''}}<br>
        <span>COD Amount :  </span> {{$orders->cod_amount??''}}<br>
        <span>Wallet Amount :  </span> {{$orders->wallet??''}}<br>
