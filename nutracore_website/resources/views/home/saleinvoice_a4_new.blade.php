        ...
        @foreach ($order_items as $i => $value)
            @php
                ...
                if(!empty($varients)){
                    ...
                    $product_subscription_price = $product->product_subscription_price ?? 0;
//                     $discount = (int)$product->product_mrp - (int)$product->product_selling_price;
                     $discount = (int)$mrp - (int)$product_subscription_price;
                }
 ...
 $product_selling_price = $product->product_selling_price ?? 0;
             if(!empty($varients)){
                ...
                if($orders->is_subscribe == 1 ){
                      $sub_total+=(int)$product_subscription_price* (int)$value->qty;
                  }else{
                      $sub_total+=(int)$product_selling_price* (int)$value->qty;
                  }
             }
