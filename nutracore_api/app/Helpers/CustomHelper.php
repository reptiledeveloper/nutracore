...
class CustomHelper
{
    ...
    public static function getProductOptions($product_id, $option_name)
    {
        ...
        if (!empty($option_name_array)) {
            foreach ($option_name_array as $val) {
                $attribu = Attributes::where('id', $val)->first();

                $attr = DB::table('attributes_products')->where('products_id', $product_id)->where('attributes_id', $val)->get()->toArray();
                if (!empty($attr)) {
                    $dbArray = [];
                    $dbArray['attribute'] = $attribu->name ?? '';
                    $dbArray['values'] = $attr;
                    $optionArr[] = $dbArray;
                }
    ...
    public static function replaceNullwithBlankString($data)
    {
    ...
    {
        $users = User::select('id', 'name')->where('is_agent', 1)->where('is_delete', 0)->get();
        return $users;
    }
    ...
    public static function getAgentData($agentID = '')
    {
        $users = [];
        if (!empty($agentID)) {
            $users = User::select('id', 'name', 'phone')->where('is_agent', 1)->where('is_delete', 0)->where('id', $agentID)->first();
        }
        return $users;
    }
    ...
    public static function getVendorProductSingleVarients1($vendor_id, $product_id, $varient_id)
    {
        $varients = VendorProductPrice::where('vendor_id', $vendor_id)->where('product_id', $product_id)->where('varient_id', $varient_id)->where('is_delete', 0)->first();
        return $varients;
    }
...
//            }
    ...
    public static function showCalendar($startDate, $endDate)
    {
        ...
        while ($start <= $end) {
            ...
            $start->addDay(); // Increment the date
        }

        return $calendar;
    }

    public static function checkAddress($addressID, $seller_id)
    {
        $is_avail = 0;
        $vendor = Vendors::find($seller_id);
        $user_address = UserAddress::find($addressID);
        if (!empty($vendor) && !empty($user_address)) {
            $distance = self::calculateDistance($vendor, $user_address);
            if (floatval($distance) <= floatval($vendor->radius)) {
                $is_avail = 1;
            }
        }
    ...
    public static function calculateDistance($vendor, $user_address)
    {
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $vendor->latitude . ',' . $vendor->longitude . '&destinations=' . $user_address->latitude . ',' . $user_address->longitude . '&mode=driving&key=AIzaSyAGs0KCxeavJJXpELDFJhtUS3PL1YrStwY',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public static function getCashFreeKey()
    {
        ...
        if (!empty($settings)) {
            if ($settings->is_live == 0) {
                $keys['key'] = $settings->cashfree_key_test ?? '';
                $keys['secret'] = $settings->cashfree_secret_test ?? '';
                $keys['is_live'] = $settings->is_live ?? '';
                $keys['payment_gateway'] = $settings->payment_gateway ?? '';
            } else {
                $keys['key'] = $settings->cashfree_key_live ?? '';
                $keys['secret'] = $settings->cashfree_secret_live ?? '';
                $keys['is_live'] = $settings->is_live ?? '';
                $keys['payment_gateway'] = $settings->payment_gateway ?? '';
            }
        }
    ...
    public static function checkProductPrice($products_id, $varient_id)
    {
        if (!empty($products_id) && !empty($varient_id)) {
            return ProductVarient::where('product_id', $products_id)->where('id', $varient_id)->first();
        } else {
            return null;
        }
    }
    ...
    public static function getRecentOrder($user): array
    {
        $product_ids = [];
        $recent_orders = Order::where('userID', $user->id)->where('status', 'DELIVERED')->limit(5)->get();
        if (!empty($recent_orders)) {
            foreach ($recent_orders as $recent_order) {
                $order_item_data = OrderItems::where('order_id', $recent_order->id)->pluck('product_id')->toArray();
                if (!empty($order_item_data)) {
                    foreach ($order_item_data as $order_item) {
                        if (!in_array($order_item, $product_ids)) {
                            $product_ids[] = $order_item;
                        }
    ...
    public static function getMostBoughtOrder($user)
    {
        $product_ids = [];
        $recent_orders = Order::where('userID', $user->id)->where('status', 'DELIVERED')->limit(50)->pluck('id')->toArray();
        if (!empty($recent_orders)) {
            $product_ids = DB::table('order_items')
                ->select('product_id', DB::raw('SUM(qty) as total_qty'))
                ->where('is_delete', 0)
                ->whereIn('order_id', $recent_orders)
                ->groupBy('product_id')
    ...
    {
        $couponData = [];
        $couponData = Offers::where('offer_code', $coupon_code)->where('end_date', '>=', date('Y-m-d'))->first();
        return $couponData;
    ...
    public static function getProductIds($vendor_id, $type = '')
    {
        $products = VendorProductPriceAlias::where('vendor_id', $vendor_id);
        if ($type == 'subscription') {
            $products->where('is_subscribed_product', 1);
        }
        $products = $products->groupBy('product_id')->pluck('product_id')->toArray();
    ...
    public static function decodeToken($token)
    {
        ...
        return self::replaceNullwithBlankString($user);
    }
    ...
    public static function fcmNotificationolddd($token, $data)
    {
    ...
    public static function fcmNotification($token, $data)
    {
        ...
        $notification = FBNotification::create($title, $body);
